...
from check_novelty import check_novelty_mol_path
import warnings
...
def generate(opt, model, SRC, TRG,n_samples=100,spec_conds=None,save=False,shift_path=''):
    warnings.filterwarnings('ignore')
    molecules, val_check, conds_trg, conds_rdkit, toklen_check, toklen_gen = [], [], [], [], [], []
    n_samples = n_samples
    ...
    if spec_conds is None:
        conds = np.array([[np.random.randint(2), np.random.randint(2), np.random.randint(2)] for i in range(n_samples)])
    else:
        conds = np.array([list(spec_conds) for _ in range(n_samples)])
    ...
    generated_molecules_valid = []
    for smiles_mol in molecules:
            if smiles_mol=='':
                continue
            if Chem.MolFromSmiles(str(smiles_mol)) is None:
                continue
            generated_molecules_valid.append(smiles_mol)

    generated_molecules_clear = list(set(generated_molecules_valid))

    df_init = pd.DataFrame(molecules, columns=['0'])
    cond_df = opt.classifier.get_cond(smiles_list = generated_molecules_clear)
    #df_init = pd.DataFrame(np.concatenate((conds, np.array(molecules).reshape(len(molecules), 1)), axis=1), columns= ['unobstructed', 'orthogonal_planes', 'h_bond_bridging', 0])
    ...
    if save:
        cond_df.to_csv(f'{opt.path_script}/valid_mols_wo_dupls{shift_path}.csv', index=False)
        df_init.to_csv(f'{opt.path_script}/generated_mols{shift_path}.csv', index=False)
    
    return cond_df


def validate(opt, model, SRC, TRG, n_samples=100, spec_conds=None, save=False, shift_path='', is_molecules=True, val_conds=None):
    ...
    for smiles_mol in molecules:
            ...
            if Chem.MolFromSmiles(str(smiles_mol)) is None:
                continue
            generated_molecules_valid.append(smiles_mol)
    valid = len(generated_molecules_valid)
    ...
    ### NOVELTY / DUPLICATES ###
    generated_molecules_clear = list(set(generated_molecules_valid))
    duplicates = n_samples - len(generated_molecules_clear)

    train_molecules = pd.read_csv(opt.train_data_csv)['0']
    novelty = len(generated_molecules_clear) - len([smi for smi in generated_molecules_clear if smi in train_molecules])
    
    ### SA SCORE ###
    sa_score = []
    for smi in generated_molecules_clear:
        sa_score.append(sascorer.calculateScore(Chem.MolFromSmiles(smi)))
    sa = len([score for score in sa_score if score <= 3])

    ### CONDITIONS ###
    if spec_conds is None:
        cond_val = np.array([list(val_conds) for _ in range(len(generated_molecules_clear))])
    else:
        cond_val = np.array([list(spec_conds) for _ in range(len(generated_molecules_clear))])
    cond_df = opt.classifier.get_cond(smiles_list = generated_molecules_clear)
    df_init = pd.DataFrame(np.concatenate((cond_val, np.array(generated_molecules_clear).reshape(len(generated_molecules_clear), 1)), axis=1),columns= opt.conditions + ['0'])
    df_total = df_init.merge(cond_df, right_on='0', left_on='0', how='right')
    init_cond = np.int_(df_total.iloc[:, :len(opt.conditions)].to_numpy())
    generated_cond = df_total.iloc[:, len(opt.conditions) + 1:].to_numpy()
    correctness = [all(i[0]==i[1]) for i in zip(init_cond, generated_cond)] # without duplicates and only valids
    
    if correctness == []:
        cond_score = 0
    else:
        cond_score = sum(correctness)
    ...
    fpgen = AllChem.GetRDKitFPGenerator()
    fps = [fpgen.GetFingerprint(Chem.MolFromSmiles(x)) for x in generated_molecules_clear]
    diversity = statistics.mean([check_diversity(Chem.MolFromSmiles(x)) for x in generated_molecules_clear])

    print('\n',
          '  examples mols:', generated_molecules_clear[:2], '\n', 
          '     valid, num:', valid, '\n', 
          'duplicates, num:', duplicates, '\n',
          '   novelty, num:', novelty, '\n', 
          '        sa, num:', sa, '\n',
          'match cond, num:', cond_score, '\n', 
          '      diversity:', round(diversity, 2), '\n')

    return cond_score, valid, novelty, duplicates, sa, diversity
