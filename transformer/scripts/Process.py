...
def create_dataset(opt, SRC, TRG, PROP, tr_te,test_mode:bool = False, shuffle=True,if_cond=True):
    ...
    if tr_te == "tr":
        ...
        df.to_csv(f"{opt.path_script}/DB_transformer_temp.csv", index=False)
    if tr_te == "te":
        print("     - # of test samples:", len(df.index))
        df.to_csv(f"{opt.path_script}/DB_transformer_temp_te.csv", index=False)
    ...
    if tr_te == "tr":
        toklenList = []
        train = data.TabularDataset(f'{opt.path_script}/DB_transformer_temp.csv', format='csv', fields=data_fields, skip_header=True)
        for i in range(len(train)):
            toklenList.append(len(vars(train[i])['src']))
        df_toklenList = pd.DataFrame(toklenList, columns=["toklen"])
        df_toklenList.to_csv(f"{opt.path_script}/weights/toklen_list.csv", index=False)
        if opt.verbose == True:
            print("     - tokenized training sample 0:", vars(train[0]))
    if tr_te == "te":
        train = data.TabularDataset(f"{opt.path_script}/DB_transformer_temp_te.csv", format='csv', fields=data_fields, skip_header=True)
        if opt.verbose == True:
    ...
    try:
        os.remove(f'{opt.path_script}/DB_transformer_temp.csv')
    except:
        pass
    try:
        os.remove(f'{opt.path_script}/DB_transformer_temp_te.csv')
    except:
        pass
    ...
    if tr_te == "tr":
        if opt.load_weights is None:
            ...
            if opt.checkpoint > 0:
                try:
                    if not os.path.isdir(f"{opt.path_script}/weights"):
                        os.mkdir("weights")
                except:
                    print("weights folder already exists, run program with -load_weights weights to load them")
                    quit()
                pickle.dump(SRC, open(f'{opt.path_script}/weights/SRC.pkl', 'wb'))
                pickle.dump(TRG, open(f'{opt.path_script}/weights/TRG.pkl', 'wb'))
