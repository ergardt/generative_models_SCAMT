...
sys.path.append(str(import_path)+'/../scripts')
...
def train_model(model, opt):
    ...
    if opt.checkpoint > 0:
        cptime = time.time()
    history_cond_score, history_valid, history_novelty, history_duplicates, history_diversity, history_sa = [], [], [], [], [], []
    ...
    for epoch in range(opt.epochs):
        total_loss, RCE_mol_loss, RCE_prop_loss, KLD_loss, accum_train_printevery_n = 0, 0, 0, 0, 0
        ...
        for i, batch in enumerate(opt.train):
            ...
            cond = torch.stack([torch.tensor([float(np.random.choice([0, 1])) for s in range(batch.batch_size)])
                                for i in opt.conditions]).transpose(0, 1).to(opt.device) # vae
        ...
        cond_score, valid, novelty, duplicates, sa, diversity = validate(
            opt=opt,
            model=model,
            SRC=opt.SRC,
            TRG=opt.TRG,
            n_samples=100,
            spec_conds=opt.spec_conds,
            val_conds=opt.val_conds) 
        ...
        data_dict = {"epochs": history_epoch, "beta": history_beta, "lr": history_lr, "total_loss": history_total_loss,
...
def main():
    ...
    data_path = import_path + '/../../data'
    parser.add_argument('-src_data', type=str, default=f'{data_path}/train_set.txt')
    parser.add_argument('-src_data_te', type=str, default=f'{data_path}/test_set.txt')
    parser.add_argument('-trg_data', type=str, default=f'{data_path}/train_set.txt')
    parser.add_argument('-trg_data_te', type=str, default=f'{data_path}/test_set.txt')
    ...
    parser.add_argument('-save_folder_name', type=str,
                        default=f'{import_path}/train_cvae')#Path were you want to save model weights per
                                                                            #epoch. Dir will created itself
    ...
    # For validate
    parser.add_argument('-cond_test_path', type=str, default=f'{data_path}/test_cond.csv')
    parser.add_argument('-cond_train_path', type=str, default=f'{data_path}/train_cond.csv')
    parser.add_argument('-train_data_csv', type=str, default=f'{data_path}/train_set.csv') # Need to check novelty of generated mols from train data
    parser.add_argument('-k', default=4)

    # Conditions
    parser.add_argument('-conditions', type=str, nargs='+', default=['qed', 'logp', 'tpsa'])
    parser.add_argument('-spec_conds', type=str, nargs='+', default=None)
    parser.add_argument('-val_conds', type=str, nargs='+', default=[1, 1, 1])
